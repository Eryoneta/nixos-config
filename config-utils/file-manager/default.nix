{
  nixosModules.file-manager = ({ config, lib, ... }: { # (NixOS Module)
    ...
    config = {
      system.activationScripts = lib.mkIf (config.file-manager.enable && config.file-manager.autoload) {
  ...
  homeModules.file-manager = ({ config, lib, ... }: { # (Home-Manager Module)
    ...
    config = {
      home.activation = lib.mkIf (config.file-manager.enable && config.file-manager.autoload) {
