{ config, userDev, auto-upgrade-pkgs, ... }@args: with args.config-utils; { # (Setup-Manager Module)
  ...
  config.modules."zsh+nx-command" = {
    ...
    attr.commands = (
      let
        version = "v1.3.1"; # Should be changed at each modification

        mkFilePath = config.modules."configuration".attr.mkFilePath;
        configurationLimit = config.modules."system-features+auto-upgrade".attr.configurationLimit;
        systemUpgradeProfileName = config.modules."system-features+auto-upgrade".attr.systemUpgradeProfileName;
        systemProfile = {
          name = "system";
          path = "/nix/var/nix/profiles/${systemProfile.name}";
          flakePath = "path:${userDev.configDevFolder or userDev.configFolder}"; # "path:" = Ignores Git repository
          preStart = "";
        };
        upgradeProfile = {
          name = "${systemUpgradeProfileName}";
          ...
          preStart = (
            let
              ...
              configLimit = configurationLimit;
            in "sudo nix-env --delete-generations +${builtins.toString (configLimit - 1)} --profile ${profilePath} || true"
          );
        };
        ...
        listGenerationsCommand = (
          utils.replaceStr "\n" "" ''
            nx-lg() {
              if [[ $2 =~ ^sys$ ]]; then
                profile=${upgradeProfile.path} ${(mkFilePath {
                  public-resource = "zsh/list-generations-fix.sh";
                  default-resource = "";
                })};
              else
                profile=${systemProfile.path} ${(mkFilePath {
                  public-resource = "zsh/list-generations-fix.sh";
                  default-resource = "";
                })};
              fi;
        ...
        deleteGenerationsCommand = (
          let
            ...
            # Note: There is also "result" folders! Generated by "build-vm", these keep stuff in the store!
          in (utils.replaceStr "\n" "" ''
            nx-dg() {
              local profilePath;
              if [[ $2 =~ ^sys$ ]]; then
      ...
      in {
        version = "echo ${version}"; # Useful for telling if the terminal got the latest one or not
        boot = (rebuildSystemCommand "boot");
        test = (rebuildSystemCommand "test");
        switch = (rebuildSystemCommand "switch");
        build-vm = (rebuildSystemCommand "build-vm");
        rollback = downgradeSystemCommand;
        upgrade = upgradeSystemCommand;
        home = rebuildHomeCommand;
        list = listGenerationsCommand;
        degen = deleteGenerationsCommand;
        trim = collectGarbageCommand;
      }
    );
    attr.buildNxCommand = commands: (
      # Note: Everything will be collapsed into a single line! Watch out for that!
      utils.replaceStr "\n" "" ''
        nx() {
          case $1 in ${
            utils.pipe commands [

              # Transforms each set into a string with its name and command
              (x: builtins.mapAttrs (commandId: commandString: (
                ''
                  "${commandId}")
                    ${commandString};
                  ;;
    ...
    setup = { attr }: {
      home = { # (Home-Manager Module)
        ...
        config.programs.zsh.initContent = ''
          # NX Command
          ${attr.buildNxCommand {
            inherit (attr.commands) version boot test switch build-vm rollback upgrade home list degen trim;
          }}
        '';
  ...
  # NX Function
  config.modules."zsh+nx-command.work" = {
    tags = [ "work-setup" ];
    attr.buildNxCommand = config.modules."zsh+nx-command".attr.buildNxCommand;
    attr.commands = config.modules."zsh+nx-command".attr.commands;
    setup = { attr }: {
      home = { # (Home-Manager Module)
        ...
        config.programs.zsh.initContent = ''
          # NX Command
          ${attr.buildNxCommand {
            inherit (attr.commands) version home trim;
          }}
        '';
